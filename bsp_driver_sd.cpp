 ...
 */
uint8_t
BSP_SD_ReadBlocks_DMA(uint8_t* pData, uint32_t ReadAddr, uint32_t NumOfBlocks)
{
 ...
 * @retval SD status
 */
uint8_t BSP_SD_WriteBlocks_DMA(uint8_t* pData,
                               uint32_t  WriteAddr,
                               uint32_t  NumOfBlocks)
{
    HAL_StatusTypeDef sd_state = HAL_OK;
...
void BSP_SD_MspInit(void)
{
    ...
    __NVIC_SetVector(SDMMC1_IRQn, (uint32_t)&__SDMMC1_IRQHandler);
    __NVIC_SetVector(DMA2_Channel5_IRQn, (uint32_t)&__DMA2_Channel5_IRQHandler);
...
static HAL_StatusTypeDef SD_DMAConfigRx(SD_HandleTypeDef* hsd)
{
    ...
    hdma_rx.Init.PeriphDataAlignment = DMA_PDATAALIGN_WORD;//DMA_PDATAALIGN_BYTE;//DMA_PDATAALIGN_WORD;
    hdma_rx.Init.MemDataAlignment    = DMA_PDATAALIGN_WORD;//DMA_PDATAALIGN_BYTE;//DMA_MDATAALIGN_WORD;
...
static HAL_StatusTypeDef SD_DMAConfigTx(SD_HandleTypeDef* hsd)
{
    ...
    hdma_tx.Init.PeriphDataAlignment = DMA_PDATAALIGN_WORD;//DMA_PDATAALIGN_BYTE;//DMA_PDATAALIGN_WORD;
    hdma_tx.Init.MemDataAlignment    = DMA_PDATAALIGN_WORD;//DMA_PDATAALIGN_BYTE;
